...
init(){
    ...
    if [ "${param}" == "reset" ]; then
        ...
        python esptool.py --port /dev/ttyUSB0 --baud 460800 write_flash --flash_size=detect -fm dio 0 ${FIRMWARE_FILE_PATH}esp8266-20171101-v1.9.3.bin
    fi
    ...
    if [ "${param}" == "files" ]; then
        ...
        # ampy --port /dev/ttyUSB0 put ${FILES_PATH}config.json
